...
- **是什么**：把一本厚书拆成「可逐章啃、每节配图」的讲解式笔记。原书讲 Gen1/2 时代的 PCI/PCIe，本笔记在保留其经典推理的同时，把每一处涉及物理速率、编码、报文格式、电源/链路状态的地方，对照 **SPEC 7.0（Gen6/7，PAM4 + Flit）** 做现代化补充。
...
- **怎么用**：
  ...
  3. 遇到「> 📘 SPEC 7.0」引用块，即为现代化补充，可对照官方 Spec 章节深入。
...
| Gen4/5 | 16 / 32 GT/s | 128b/130b（NRZ） | 更严格的信号完整性、Retimer |
| Gen6 | 64 GT/s | **PAM4** | **FEC + Flit 模式**（强制）、L0p |
| Gen7 (7.0) | 128 GT/s | PAM4 | 沿用 Flit + FEC，进一步优化 |

> 📘 **贯穿全书的三条现代化主线**：①编码从 8b/10b→128b/130b→PAM4（第07章）；②Gen6 起进入 **Flit（Flow Control Unit）模式**，TLP/DLLP 封装与流控、序都随之调整（第06/07/09/11章）；③电源与链路状态新增 L1 子状态、L0p 等（第08章）。
...
5. **📘 SPEC 7.0 现代化对照** —— 该章涉及的新版演进（Flit/PAM4/新字段等），并注 Base Spec 章节号。
6. **常见误区 / FAQ** —— 易错点、面试/实战高频问题。
7. **小结** —— 一图/一表收束。
8. **与 SPEC 章节对照表** —— 本章内容 ↔ Base Spec 7.0 章节映射。

> 大纲阶段的章节文件已按此骨架排布：`导读 → 术语表 → 小节提要（对应 3/4）→ SPEC 7.0 现代化补充清单 → 配图清单 → SPEC 章节对照`。正文阶段在此骨架上补全 §5.1 全部 8 节。
...
> 首批产出为 **CLAUDE.md + 全部 15 章详细大纲**（每章已含术语表）；各章正文将按 §5.1 骨架逐章填充，进度见 [TODO.md](TODO.md)。
