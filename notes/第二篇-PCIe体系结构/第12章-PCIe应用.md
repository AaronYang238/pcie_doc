...
>
> **现代化补充**：原书成书于 Gen1/2，带宽账是按 8b/10b（编码先亏 20%）算的。本章用当代速率（Gen4/5/6）重算带宽与延时的数量级，并说明为什么高速率下 **10-bit/14-bit Tag**（[第06章](第06章-事务层.md)）成了跑满读吞吐的前提。本章按 [CLAUDE.md §5.1](../../CLAUDE.md) 八节骨架展开，配 4 张图。
...
在途请求数受 **Tag 数**限制（8-bit Tag = 256 个），高速率×高延迟链路下 256 个可能不够——这正是需要 10-bit/14-bit Tag 的现代化动因（见 §SPEC 补充）。
...
> 📘 **带宽重算：从 8b/10b 到 128b/130b**。原书按 Gen1 的 **8b/10b** 编码算账——编码层就先亏掉 20%。Gen3 起改用 **128b/130b**（[第07章](第07章-数据链路层与物理层.md)），编码开销骤降到约 **1.5%**。所以现代的带宽账要换基数：Gen4 x16 裸速 ≈ 31.5 GB/s（单向），乘以 TLP 效率 ~90% ≈ 28 GB/s 可用；Gen5 x16 ≈ 63 GB/s、Gen6 ≈ 121 GB/s（裸速）。图中深色面板给出了这组近似值。

> 📘 **在途并发：10-bit / 14-bit Tag 是跑满读吞吐的前提**。`读吞吐 ≈ 在途请求 × MRRS ÷ RTT`，而在途请求数受 Tag 数封顶。原书 8-bit Tag 只有 256 个；在高速率×高延迟（尤其经 Retimer/长链路）下，256 个在途根本填不满"带宽×延迟"管道。SPEC 引入 **10-bit Tag（1024 个）和 14-bit Tag（16384 个）**（[第06章](第06章-事务层.md)），正是为了让现代高带宽链路的读也能跑满。（Base Spec §2.2.6.2 Tag 相关）

> 📘 **Flit 模式的开销结构完全不同**。Gen6 起强制 **Flit（Flow Control Unit）模式**（[第06/07章](第06章-事务层.md)）：TLP 不再各自带成帧和 LCRC，而是被打包进定长 Flit，CRC 与 **FEC（前向纠错）** 按 Flit 固定占比。所以本章"每个 TLP 26B 固定开销"的模型只适用于**非 Flit（Gen5 及以前）**；Flit 模式下开销是**每 Flit 摊销**的固定百分比，带宽账需按 Flit 结构另算。这也是为什么图中特别注明"Flit 模式开销结构不同，需另算"。
...
| Tag 扩展（在途并发） | 10-bit / 14-bit Tag（Tag 字段与 Extended Tag） |
| 有效带宽 / 性能 | Performance 相关（TLP 开销模型） |
| Flit 模式开销 | Flit Mode（Gen6+，与非 Flit 开销结构不同） |
