...
> **导读**：设备干完活（比如网卡收到一个包、NVMe 读完一块数据），得有办法通知 CPU"来处理我"。这个"通知"就是**中断**。上一章 [第11章](第11章-总线的序.md) 结尾我们已经剧透了关键一句——**在 PCIe 里，"中断"本质上就是一次特殊的存储器写 TLP**：设备往一个约定好的地址写一个约定好的数，RC 认出"这是中断"，转成对目标 CPU 的中断。这就是 **MSI（Message Signaled Interrupt，消息信号中断）**。
>
...
演进主线：**INTx（专用线、共享）→ MSI（带内、少量向量）→ MSI-X（大量、每向量独立）→（现代）IMS（设备私有、海量）**。下一章 [第12章](第12章-PCIe应用.md) 会用一块示例卡把 DMA + MSI 的完整流程走一遍，让本章的中断落到实处。
